...
Both clients share their backing servers from [demo-servers/](./demo-servers) rather than each keeping a copy. Everything through item 5 stays on `localhost`; items 6–7 are the only examples that touch a genuinely remote, third-party-hosted MCP server.
